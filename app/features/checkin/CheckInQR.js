    ...
    const onSuccess = async (e) => {
        ...
        if (json.uid) {
            ...
            if (res.status !== 200) {
                ...
                createAlert("This person never submitted an application to ", hackathon.name);
            } else if (res.json.applications[0].status !== 'CONFIRMED') {
                createAlert("This person's application is not confirmed yet. Current status: ", res.json.applications[0].status);
            } else {
        ...
        <View style={[dynamicStyles.backgroundColor, {flex: 1}]}>
            <View style={{ alignItems: 'center', paddingTop: 40 }}>
                ...
                <QRCodeScanner
                    ...
                    onRead={onSuccess}
                    cameraStyle={{ width: Dimensions.get("window").width - 30, alignSelf: 'center' }}
                />
            </View>
        </View>
    )
