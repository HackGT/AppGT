import { COMPLETE_QUESTION, COMPLETE_HINT, SET_FROM_STORAGE } from './ScavHuntActionTypes'
...
function completeHint(state, item) {
    const newState =  { ...state, completedHints: state.completedHints.concat([item.id + '-' + item.code])}
    console.log('in reducer: ', state, item, newState, JSON.stringify(newState.completedHints))
