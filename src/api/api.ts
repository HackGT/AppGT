...
export const checkoutSwagItem = async (
  token: string,
  userId: string,
  swagItemId: string,
) => {
...
export const getHexathonUser = async (
  token: string,
  hexathonId: string,
  uid: string,
) => {
...
export const getEvents = async (token: string) => {
  try {
    ...
    eventJson.sort(
      (a: any, b: any) => Date.parse(a.startDate) - Date.parse(b.startDate),
    );
    return { status: response.status, eventJson };
  } catch (err) {
    return {
      status: 500,
      json: { message: "Network error when getting events" },
    };
  }
};
...
export const getBlocks = async (token: string) => {
  try {
    ...
    return {
      status: 500,
      json: { message: "Network error when getting blocks" },
    };
  }
};
...
export const getScavengerHunt = async (token: string) => {
  try {
    ...
    let blockJson = await response.json();
    blockJson = blockJson.filter((block: any) => block.display === "MOBILE");
    const scavBlock = blockJson.find(
      (block: any) => block.slug === "scavenger-hunt",
    );
    ...
    return {
      status: 500,
      json: { message: "Network error when getting scavenger hunt" },
    };
  }
};
