...
const eventProps = [
    makeEvent("Event 1", "Really cool event.", [{key: "Tag1"}, {key: "Tag2"}, {key: "Tag3"}]),
    makeEvent("Event 2", "Even cooler event.", [{key: "Tag4"}, {key: "Tag5"}, {key: "Tag6"}]),
    makeEvent("Event 3", "This event will blow your socks off.", [{key: "Tag7"}, {key: "Tag8"}, {key: "Tag9"}])]
...
export default class Schedule extends Component<Props> {
    ...
    render() {
        ...
        const cardEvent = ({ item, index, section: { title, data } }) => (
            <View key={index}>
                <ScheduleCard item={item} onClick={this.onSelectEvent} title={item.title} tags={item.tags}>{item.desc}</ScheduleCard>
                {vSpace}
            </View>
        )
